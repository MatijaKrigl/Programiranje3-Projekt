...
namespace ClovekNeJeziSe
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            timerHideDice.Interval = 50;
            timerHideDice.Tick += new EventHandler(timerHideDice_Tick);
            zamikTimer.Interval = 20;
        ...
        private void gumbZaMetKocke_Click(object sender, EventArgs e)
        {
            ...
            var figuresOnBoard = trenutniIgralec.Figure.Where(f => f >= 0).ToList();
            var figuresInStart = trenutniIgralec.Figure.Select((value, index) => new { value, index })
                                .Where(x => x.value < -10).Select(x => x.index).ToList();
            timerHideDice.Start();

            if (rezultatKocke == 6)
            {
                if (figuresOnBoard.Count > 0 && figuresInStart.Count > 0)
                {
                    // Igralec lahko izbere med premikom figure na plošči ali figuro iz začetne pozicije
                    int selectedFigureIndex = IzberiFiguro(trenutniIgralec, true); // Drugi parameter pove, da igralec lahko izbere iz začetnih pozicij
                    if (figuresInStart.Contains(selectedFigureIndex))
                    {
                        igra.PremakniFiguro(trenutniIgralec, selectedFigureIndex, rezultatKocke); // Aktiviraj figuro iz začetne pozicije
                    }
                    else
                    {
                        igra.PremakniFiguro(trenutniIgralec, selectedFigureIndex, rezultatKocke); // Premakni figuro na plošči
                    }
                    gumbZaMetKocke.Enabled = true;
                }
                else if (figuresInStart.Count > 0)
                {
                    // če so samo figure v začetnih pozicijah
                    int selectedFigureIndex = figuresInStart.First();
                    igra.PremakniFiguro(trenutniIgralec, selectedFigureIndex, rezultatKocke);
                }
                else if (figuresOnBoard.Count > 0)
                {
                    // če so samo figure na plošči
                    if (figuresOnBoard.Count > 1)
                    {
                        int selectedFigureIndex = IzberiFiguro(trenutniIgralec);
                        igra.PremakniFiguro(trenutniIgralec, selectedFigureIndex, rezultatKocke);
                    }
                    else
                    {
                        int indexFigure = trenutniIgralec.Figure.ToList().IndexOf(figuresOnBoard.First());
        ...
        private int IzberiFiguro(Igralec trenutniIgralec, bool includeStart = false)
        {
            // Pridobimo indekse figur, ki so na plošči ali v začetnih pozicijah
            var options = trenutniIgralec.Figure
                .Select((value, index) => new { value, index })
                .Where(x => (includeStart && x.value < -10) || x.value >= 0)
                .Select(x => x.index)
                .ToArray();

            string[] optionsText = options.Select(index => $"Figura {index + 1}").ToArray();

            using (var selectPieceForm = new IzberiFiguroForm(optionsText, this, trenutniIgralec.Id))
            {
                if (selectPieceForm.ShowDialog() == DialogResult.OK)
                {
                    return options[selectPieceForm.SelectedIndex];
                }
            }

            // Default to the first option if the user cancels
            return options[0];
        }

        private void timerHideDice_Tick(object sender, EventArgs e)
        {
        ...
        private void PobarvajPolja(Graphics g, bool barvanje, int barva)
        {
